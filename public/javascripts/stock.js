...
function setStocks(strStocks) {
	...
	stockGetUrl = "quotes?list=" + strStocks;
	stockGetInfos = [];
...
function getStockInfo(stockCode, f){
	try {
		...
		xhr.open("GET", "quotes?list=" + stockCode, true);
		xhr.onreadystatechange = function() {
			var stockInfo = undefined;
			...
			if (xhr.readyState == 4) {
				...
				if(elements.length > 5) {
					try {
						...
						//var dataview = new DataView(elements[3]);
						//stockName = decoder.decode(dataview);
						//delete decoder; delete dataview;
						//stockName = elements[3];
					} catch(e) { 
						console.error(e); 
						console.log(elements);
...
function updateStocksNew(codeList, f) {
	var Url = "quotes?list=" + codeList.join(",");
	try {
		var xhr = new window.XMLHttpRequest();
